...
def get_dataset(dataset_name):
    ...
    if dataset_name == 'MNIST':
        ...
        # train_dataset = torchvision.datasets.FashionMNIST(root=DATA_LOCATION, train=True,
        #                                                   transform=transforms.ToTensor(),
        #                                                   download=True)
        # test_dataset = torchvision.datasets.FashionMNIST(root=DATA_LOCATION, train=False,
        #                                                  transform=transforms.ToTensor(),
        #                                                  download=True)
...
def get_dataloaders(dataset_name, down_sampled_size=None, train_subset=1.0, train_batch_size=256):
    ...
    if dataset_name == 'MNIST':
        ...
        train_dataset, test_dataset = get_dataset('FASHION')
        train_size = 50000
        val_size = 10000
        full_training_size = 60000
    else:
...
def load_model(work_dir, config_id):
    ...
    if len(g) == 0:
        ...
        model = None
        with file_io.FileIO(g[0], mode='rb') as f:
            model = pickle.load(f)
...
def save_model(work_dir, budget, config_id, model):
    ...
    with file_io.FileIO(model_file_name, mode='w+') as f:
        pickle.dump(model, f)
