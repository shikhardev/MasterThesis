...
class EpochWithIncreasingTrainset(Master):
    def __init__(self, configspace, eta=2,
                 starting_trainset_budget=0.0625, time_deadline=None, max_trainset_iterations=None,
        ...
        # Algo Type
        self.algo_type = 'InfiniteEpochWithIncreasingTrainset'
    ...
    def check_terminal_condition(self):
        ...
        #     # TODO: current_trainset_iterations is not modified anywhere right now
    ...
    def get_next_iteration(self, iteration, iteration_kwargs={}):
        if len(self.run_details) > 2 and self.check_terminal_condition():
            return None

        # If we have exhausted the entire trainingset and we have still not terminated,
        # increasing the epoch_multiplier will essentially increase the number of epochs in the budget
        if iteration % self.max_SH_iter == 0:
            self.update_aux_budget_tracker(iteration)
            self.trainset_budget, self.epoch_multiplier = self.get_next_auxilary_budgets(self.trainset_budget,
                                                                                         self.epoch_multiplier)
            self.trainset_budget = min(1, self.trainset_budget)
