...
class FashionWorker(Worker):
    def __init__(self, epc=None, **kwargs):
        super().__init__(**kwargs)
        self.trainset, self.testset = get_dataset('FASHION')
        self.trainset, self.valset = torch.utils.data.random_split(self.trainset,
                                                                   [FASHION_TRAINSET_SIZE, FASHION_VAL_SIZE])
    ...
    @staticmethod
    def get_configspace():
        return get_config_space('FASHION')
    ...
    def compute(self, config_id, config, budget, working_directory):
        ...
        # elif self.hpo_optimizer.algo_type == 'time-based' or \
        #         self.hpo_optimizer.active_test_algo == 'time-based':
