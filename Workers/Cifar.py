...
class CifarWorker(Worker):
    def __init__(self, epc=None, **kwargs):
        super().__init__(**kwargs)
        self.trainset, self.testset = get_dataset('CIFAR10')
        self.trainset, self.valset = torch.utils.data.random_split(self.trainset, [CIFAR_TRAINSET_SIZE, CIFAR_VAL_SIZE])
    ...
    @staticmethod
    def get_configspace():
        return get_config_space('CIFAR10')
    ...
    def compute(self, config_id, config, budget, working_directory):
        ...
        if DEBUG:
            ...
            print('\n' * 10)
            print('-' * 20)
            print('Executing Time based!!')
            print('Duration: ', dur)
            print('Trainset Budget: ', trainset_budget)
            print('epochs_for_time_budget: ', epochs_for_time_budget)
            print('-' * 20)
            print('\n' * 10)
...
if __name__ == '__main__':
    worker = CifarWorker(run_id='0', epochs_per_budget=4)
