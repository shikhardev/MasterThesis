...
def data_to_mat(filename, vocab, tag_to_number, window_size=1, start_symbol=u'UUUNKKK',
                one_hot=False, return_labels=True, sample_percentage=1.0):
    '''
    :param sample_percentage: % of the dataset to be returned.
    ...
    x = np.array(x)
    y = np.array(y)
    ret_len = int(sample_percentage * len(x))
    return x[:ret_len], y[:ret_len]
